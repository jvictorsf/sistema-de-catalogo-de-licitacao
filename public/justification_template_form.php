...
$template = $id ? find_justification_template($id) : null;
$categories = get_parent_categories();

if ($id && !$template) {
    http_response_code(404);
    exit('Justificativa não encontrada.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        if ($template) {
            update_justification_template((int) $template['id'], $data);
        } else {
            create_justification_template($data);
        }
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h3 mb-1">
            <?= $template ? 'Editar justificativa' : 'Nova justificativa' ?>
        </h1>
        <p class="text-muted mb-0">
            Cadastre textos reutilizáveis para o campo de justificativa.
        </p>
    </div>
