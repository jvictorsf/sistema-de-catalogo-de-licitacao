...
$result = calculate_project_item_quantity_memory($exampleInput, 44, 42);

quantity_memory_assert_close(42.0, $result['approved_quantity_snapshot'] ?? null, 'A memoria deve preservar a quantidade aprovada.');
quantity_memory_assert_close(44.0, $result['requested_quantity_snapshot'] ?? null, 'A memoria deve preservar a quantidade solicitada.');
...
quantity_memory_assert_close(7.0, $result['additions_total'] ?? null, 'O total de acrescimos deve ser calculado corretamente.');
quantity_memory_assert_close(6.0, $result['deductions_total'] ?? null, 'O total de deducoes deve ser calculado corretamente.');
quantity_memory_assert_close(43.0, $result['calculated_quantity'] ?? null, 'A quantidade calculada deve bater com o exemplo.');
quantity_memory_assert_close(43.0, $result['final_quantity'] ?? null, 'A quantidade final deve bater com o exemplo.');
...
quantity_memory_assert_close(
    43.0,
    project_item_effective_quantity(array_merge($result, ['quantity_memory_id' => 1])),
    'A quantidade efetiva deve usar o valor final da memoria persistida.'
);
...
quantity_memory_assert_close(0.0, $result['calculation_data']['technical_loss']['calculated_quantity'] ?? null, 'Perdas tecnicas devem permanecer zeradas no exemplo.');
quantity_memory_assert_close(0.0, $result['calculation_data']['other_additions']['quantity'] ?? null, 'Outros acrescimos devem permanecer zerados no exemplo.');
